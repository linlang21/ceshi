...
> **当前版本：** v38.1（C# .NET Framework 4.x 单文件 EXE）｜payload v38.0
> **最后更新：** 2026-06-29
...
- **功能**
  ...
  - NPC 节奏游戏、象棋秒赢、投壶圈变大、终止过场动画、关闭安全标志
  - 剧情速度
- **Buff**
...
- **传送**（独立标签页，v38.0 从工具 tab 拆分）
  - 坐标管理：坐标列表、选择/打开坐标文件、自动瞬移、自动按F、带千斤顶
  - 坐标保存：手动输入 X/Z/Y 坐标、保存到桌面
  - 坐标Hook：初始化Hook（Frida Interceptor 抓 rbx 基址）、验证AOB
  - 启用传送：默认关闭，勾选后才能执行传送操作（安全开关）
...
|------|------|------|------|------|
| 初始 | 选择目录 | `BtnBrowse_Click(...)` | - | 选择游戏目录并更新通信路径 |
| 初始 | 启动游戏 | `BtnStartGame_Click(...)` | - | 启动 `yysls.exe` |
| 初始 | 注入工具 | `BtnInject_Click_B(...)` | - | 生成 `svc.cfg`、创建 MMF、注入 `core.dll` |
| 初始 | 刷新状态 | `CheckState()` | - | 刷新就绪/文件状态 |
...
| 功能 | 自动拾取 | `BuildLoopFeatureLua(...)` | `loot_once` | 原“辅助”页功能已迁移 |
| 功能 | 一键恢复 | `BuildYyLaoLiuLua(...)` | `yy_recover` | 原“辅助”页功能已迁移 |
| 功能 | NPC节奏游戏 | `BuildGameFeatureLua(...)` | `rhythm_game` | 原“辅助”页功能已迁移 |
| 功能 | 象棋秒赢 | `BuildGameFeatureLua(...)` | `chess_win` | 原“辅助”页功能已迁移 |
| 功能 | 投壶圈变大 | `BuildGameFeatureLua(...)` | `pitch_pot_easy` | 原“辅助”页功能已迁移 |
| 功能 | 终止过场动画 | `BuildCombatExperimentLua(...)` | `cutscene_kill` | 原“辅助”页功能已迁移 |
| 功能 | 关闭安全标志 | `BuildLoopFeatureLua(...)` | `stealth_flags` | 原“辅助”页功能已迁移 |
...
| Buff | 攻击Buff | `BuildCombatExperimentLua(...)` | `atkbuff_combo` | 当前不是 `BuildLoopFeatureLua` |
| Buff | 防御Buff | `BuildCombatExperimentLua(...)` | `defbuff` | 当前不是 `BuildLoopFeatureLua` |
| Buff | 最小Buff | `BuildLoopFeatureLua(...)` | `minimal_buff` | 默认 `{30372,70063,1053070}` |
| Buff | 采集Buff | `BuildLoopFeatureLua(...)` | `gather_buff` | 读 `buff_config.txt` |
| Buff | 辅助Buff | `BuildLoopFeatureLua(...)` | `aux_buff` | 读 `buff_config.txt` |
| Buff | 未知Buff | `BuildLoopFeatureLua(...)` | `unknown_buff` | 读 `buff_config.txt` |
| Buff | 移除全部Buff | `BuildLoopFeatureLua(...)` | `remove_all_buffs` | 当前主移除入口 |
| Buff | 老六移除Buff | `BuildYyLaoLiuBuffToolLua(...)` | `yy_remove_buffs` | 老六脚本系独立入口 |
| Buff | 循环强力Buff | `BuildLoopFeatureLua(...)` | `loop_buff` | 再点停止 |
| Buff | 循环防御Buff | `BuildLoopFeatureLua(...)` | `loop_defense` | 再点停止 |
| Buff | 循环自动拾取 | `BuildLoopFeatureLua(...)` | `loop_loot` | 再点停止 |
| Buff | 循环自动恢复 | `BuildLoopFeatureLua(...)` | `loop_recover` | 再点停止 |
| 工具 | 锁体力消耗 | `BuildCombatExperimentLua(...)` | `stamina_lock` | 资源类 |
| 工具 | 无限潜水资源 | `BuildCombatExperimentLua(...)` | `stamina_dive` | 资源类 |
| 工具 | 清空战斗资源 | `BuildCombatExperimentLua(...)` | `stamina_empty` | 资源类 |
| 工具 | 恢复体力设置 | `BuildCombatExperimentLua(...)` | `stamina_reset_all` | 资源类 |
| 工具 | 攻击倍率 | `ApplyAttackMultiplierSelection()` → `BuildCombatExperimentLua(...)` | `atk_mul_2/4/8` | 下拉框生成 mode |
| 工具 | 还原攻击倍率 | `BuildCombatExperimentLua(...)` | `atk_mul_reset` | 还原 `do_direct_damage` |
| 工具 | 攻击速度 | `ApplyAtkSpeedSelection()` → `BuildCombatExperimentLua(...)` | `atk_speed_1.5/3/5/7.5/10/30` | 下拉框生成 mode |
| 工具 | 还原攻击速度 | `BuildCombatExperimentLua(...)` | `atk_speed_reset` | 固定还原 |
| 工具 | 记录坐标 | `BuildLogPositionLua()` | `log_pos` | 已从构造函数抽出独立 builder |
...
| 传送 | 启用传送 | `chkEnableTeleport` | - | 默认关闭，勾选后才能执行传送操作 |
